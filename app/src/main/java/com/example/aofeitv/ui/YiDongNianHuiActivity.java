package com.example.aofeitv.ui;
...
import com.example.aofeitv.MyApplication;
import com.example.aofeitv.R;
import com.example.aofeitv.beans.AllUserBean;
...
import com.example.aofeitv.beans.ChuanJianUserBean;
import com.example.aofeitv.beans.MoShengRenBean2;
import com.example.aofeitv.beans.MoShengRenBeanDao;
import com.example.aofeitv.beans.QianDaoIdDao;
import com.example.aofeitv.beans.ShiBieBean;
import com.example.aofeitv.beans.TanChuangBean;
import com.example.aofeitv.beans.TanChuangBeanDao;
import com.example.aofeitv.beans.TuPianBean;
import com.example.aofeitv.beans.User;
import com.example.aofeitv.beans.WBBean;
import com.example.aofeitv.interfaces.RecytviewCash;
import com.example.aofeitv.service.AlarmReceiver;
import com.example.aofeitv.tts.control.InitConfig;
import com.example.aofeitv.tts.control.MySyntherizer;
import com.example.aofeitv.tts.control.NonBlockSyntherizer;
import com.example.aofeitv.tts.listener.UiMessageListener;
import com.example.aofeitv.tts.util.OfflineResource;
import com.example.aofeitv.utils.DateUtils;
import com.example.aofeitv.utils.GlideRoundTransform;
import com.example.aofeitv.utils.GsonUtil;
import com.example.aofeitv.utils.Utils;
import com.example.aofeitv.view.WrapContentLinearLayoutManager;
...
//					break;
			...
			if (msg.arg1==1){
				...
				try {
					...
					bean.setRemark(dataBean.getRemark()==null? "":dataBean.getRemark());
		...
		private MyAdapter(int layoutResId, List<TanChuangBean> data) {
			super(layoutResId, data);
			 myOptions = new RequestOptions()
					 .transform(new GlideRoundTransform(YiDongNianHuiActivity.this,10));
					//.centerCrop();
...
//			}else {
				...
				switch (item.getType()) {
					...
					case 0:
						...
						if (bumen.contains("访客")){
							//设置成领导背景
							//imageView.setBackgroundColor(Color.parseColor("#00000000"));
							toprl.setBackgroundResource(R.drawable.bg2);
							name.setTypeface(typeFace1);
						//	zhuangtai.setTypeface(typeFace1);
							String xb=item.getGonghao();
							if (item.getGonghao()==null){
								xb="";
							}
							String s3=baoCunBean.getFkHyc()==null?"":baoCunBean.getFkHyc();
							name.setText(s3.replace("@",item.getName()).replace("#",xb));

						//	synthesizer.speak(item.getName()+"欢迎您莅临会议");
						}else if (bumen.contains("维护人员")){
							//imageView.setBackgroundColor(Color.parseColor("#00000000"));
							toprl.setBackgroundResource(R.drawable.bg2);
							name.setTypeface(typeFace1);
						//	zhuangtai.setTypeface(typeFace1);
							...
							String s3=baoCunBean.getWhHyc()==null?"":baoCunBean.getWhHyc();
							name.setText(s3.replace("@",item.getName()).replace("#",xb));
						//	zhuangtai.setVisibility(View.GONE);
						//	synthesizer.speak(item.getName()+"签到成功");
						}else {
							//imageView.setBackgroundColor(Color.parseColor("#00000000"));
							toprl.setBackgroundResource(R.drawable.bg1);
							name.setTypeface(typeFace1);
							//	zhuangtai.setTypeface(typeFace1);
							String xb=item.getGonghao();
							if (item.getGonghao()==null){
							...
							String s3=baoCunBean.getYgHyc()==null?"":baoCunBean.getYgHyc();
							name.setText(s3.replace("@",item.getName()).replace("#",xb));
						}
		...
		//	}
			...
			//文字
			LinearLayout.LayoutParams lp5 = (LinearLayout.LayoutParams) name.getLayoutParams();
			lp5.width=dw-(dw/4)-(dw/10);
			lp5.leftMargin=(dw/4);
			lp5.rightMargin=(dw/10);
			lp5.height=(dh/2)-190;
			name.setLayoutParams(lp5);
			name.invalidate();
...
//						"&mcode=21:21:DA:F2:00:51:3B:AB:C4:E6:19:18:31:C6:98:CA:D6:7B:44:AE;com.example.aofeitv");
